...
(setq enh-ruby-indent-tabs-mode nil) ;; or "t"
...
(bind-key "M-s f" 'find-name-dired)

(bind-key "C-x   C-f" 'find-file)
(bind-key "C-x x C-f" 'find-file-in-project-by-selected)
(bind-key "C-x   s-f" 'find-file-in-project)
#+END_SRC
...
(bind-key "C-c g c m" 'magit-commit)
(bind-key "C-c g c o" 'magit-checkout)
(bind-key "C-c g l" 'magit-pull)
(bind-key "C-c g p" 'magit-pull)
(bind-key "C-c g d" 'magit-diff)
